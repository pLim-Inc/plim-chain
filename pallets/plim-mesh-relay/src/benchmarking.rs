...
//! weight `Weight::from_parts(50_000_000, 4096)` declared in `lib.rs`. That
//! placeholder is conservative for the worst case (1 KiB payload, ed25519
//! verification, blake2_256 hashing, and three O(1) storage writes: one
//! `RelayedTransactions` map entry, the `RelayedQueueCount` value, and one
//! `RelayedHashIndex` map entry).
